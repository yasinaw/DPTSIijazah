<?PHP
	...
	<table>
		<tr>
			<td width=\"159mm\"><img src=\"/simwisuda/inc/atas.png\" style=\"width: '2500'; height: 80\" ></td>
		</tr>
		<tr>
		<td height=\"200mm\">
			<table width=\"100%\">	
					...
					<td>
						...
						<tr>
							<td valign=\"middle\" width=\"2%\" >
								<!-- GAMBAR BORDER KIRI -->
								<br>
								<img src=\"/simwisuda/inc/side.png\" style=\"width: 30; height: 1950\" >
								
							</td>
							<td style=\"width:'94%'; height:197mm\">
							<table  width='100%'>
								<tr>
									<td style=\"width:'100%'; height:195mm\">";	
			...
			foreach ($fak_jur_prog->result_array() as $row)
			{
				...
				$fakultas = $row['NAMAFAKULTAS'];
				$jurusan = $row['NAMAJURUSAN'];
				switch(substr($row['KODEPRODI'],0,4)) {
					case '1303':
				...
				/*
				if(substr($row['KODEPRODI'],0,4)=='2403') {
					$jurusan = 'T. Metrologi dan Instrumentasi';
				}
				if(substr($row['KODEPRODI'],0,4)=='4130' || substr($row['KODEPRODI'],0,4)=='4120') {
					$jurusan = 'Teknologi Kelautan';
				}
				*/
				//$program = $row['PS_Nama'];
			...
			if($program == 'S3'){
				...
				$nama_judul='Judul Tesis:';	
			}
			else if ($program == 'Profesi') {
				$nama_program = 'Profesi';
				$nama_judul = 'Judul TA:';
			}
			else if($program == 'S1'){
            ...
            {	
				...
				/*SIM WISUDA*/
				//$nrptbl[$ctr] = substr($row['NRP'],0,2).substr($row['NRP'],4,3) == '00111' ? $row['NRP_BARU'] : $row['NRP'];
				$nrptbl[$ctr] = substr($row['NRP'],4,3) == '111' ? $row['NRP_BARU'] : $row['NRP'];
				$nrptbl2[$ctr]=$row['NRP_URUT'];
				$namatbl[$ctr]=ucwords(strtolower($row['NAMA']));
				$ipktbl[$ctr]=$row['IPK'];
				$lamastuditbl[$ctr]=$row['LAMASTUDI'];
				$tanpa_predikat = array('TP','T');
				$predikat[$ctr]= in_array($row['PREDIKAT'], $tanpa_predikat) ? '-' : $row['PREDIKAT'];
				$lulusanketbl[$ctr]=substr($row['NRP'],4,3);
				
				if($predikat[$ctr] == 'D' or $predikat[$ctr] == 'P'){
					$predikat[$ctr] = 'P';
				}
				else if($predikat[$ctr] == 'S'){
					$predikat[$ctr] = 'SM';
				}
				else if($predikat[$ctr] == 'T') {
					$predikat[$ctr] = '-';
				}
					...
					<h3 align=\"center\">
						...
						Tanggal Wisuda : '.$tanggal_id.'
							...
							<tr>
								...
								<td style="width:17%" class="atas"></td>
								<td style="width:38%" class="atas"></td>
							...
							<tr>
								...
								<td style="width:17%"><br/>NRP</td>
								<td style="width:38%"><br>N A M A </td>
		...
		<td height=\"200mm\">
			<table width=\"100%\">	
				...
				if ($var_tempnrp!=substr($nrptbl2[$j],0,2).substr($nrptbl2[$j],4,3) && ($program == 'S2'))
				//if ($program == 'S2')
				{
					$cekbidangkeahlian=$cekbidangkeahlian+1;
					$var_kodeprodi=substr($nrptbl2[$j],0,2).substr($nrptbl2[$j],4,3);
				...
				<tr>
					<td style="width:1%"></td>
					<td style="width:5%">'.$no.'</td>
					<td style="width:17%">'.$nrptbl[$j].' </td>
					<td style="width:38%" align="left">'.$namatbl[$j].'</td>
				...
				$var_tempnrp=substr($nrptbl2[$j],0,2).substr($nrptbl2[$j],4,3);							
				
				/*PERHITUNGAN IP DAN LAMA STUDI RATA-RATA*/
				if($program == 'S3' || $program == 'S2' || $program = 'Profesi')//Jika Program S-3
				{
					$ip=$ip+$ipktbl[$j];
					$iprata2=$ip/$no;
				...
				if ($cekbidangkeahlian<=0)
				{
					...
					if((substr($nrptbl[$j],0,2)=='33' && substr($nrptbl[$j],4,2)=='10'))
						$jumbaris = 30;
				}
				else
				{
					...
					if(((substr($nrptbl[$j],0,2)=='21' && substr($nrptbl[$j],4,2)=='20'))) {
						$jumbaris=25;
					}
				...
				{									
					...
					<td width="2%" height="100%">
						...
						<br>
						<img src="/simwisuda/inc/sidekanan.png" style="width: 30; height: 1950" >
					</td>
				</tr>
				<tr>
					<td colspan="4" >
            ...
            {	
				...
				$nrpdata[$ctr]=$row['NRP'];
				$nrpdatafoto[$ctr] = $row['NRP'];
				if(substr($nrpdata[$ctr],2,2) >= '17') {
					$nrpdata[$ctr] = $row['NRP_BARU'];
				}
			...
			<table>
				<tr>	
					<td width=\"158mm\" height=\"215mm\">	
						...
						<tr>
							<td valign=\"middle\" width=\"4mm\" style=\"height:180mm\">
								<!-- GAMBAR BORDER KIRI -->
								<br>
								<img src=\"/simwisuda/inc/side.png\" style=\"width: 20; height: 1030\" >
								
							</td>
							<td height:\"190mm\" style=\"width:148mm\">
								<!-- table green-->		
								<!--table width='100%' style=\"border:3px solid green\"-->
								<table width=\"100%\">
								<tr>
									<!--td style=\"width:3%\"></td-->
									<td style=\"width:'100%'; height:175mm\">"; 
					/*Border layout kolom kiri kanan*/
					$html=$html.'<table height="185mm" style="width: 100%">
							<tr >';
					...
					if (($ukurankertas=='ITS PAPER') || ($ukurankertas=='B5'))
						...
						{		
							...
							<td style="width: 70mm; height:185mm">';
							//$html=$html.'<td style="width: 50%; border:3px solid pink">';
						}
						...
						<tr>
							<td style="height:65mm">
								<table width="98%">
									<tr>
										<td width="2.2cm">';							
								//if($valarr[0] == '3810') $valarr[0] = '3410';
								//if($valarr[0] == '2810') $valarr[0] = '2510';
								$filename ='../simwisuda/inc/Wisudawan/'.$periodewisuda.'/'.$valarr[0].'/'.$nrpdatafoto[$i].'.jpg';
								...
								/*Data NRP, Nama, Foto Kolom Kiri*/
								 $html=$html.'	
										<div >
											...
											NRP: '.$nrpdata[$i].'<br>
											TTL: '.strtoupper($tgllahir[$i]).'<br>
											ORTU: '.strtoupper($ortu[$i]).'<br>
											Alamat: '.strtoupper($alamatdata[$i]).'<br>
											Email: <font size:"7pt">';
												if(strlen($email[$i])>22){
													$html=$html.'	<br>'.$email[$i];
												}
												else {
												$html=$html.$email[$i];
												}
					...
					<table width="100%">
						<tr>
								<td width="45%" align="right"></td>
								<td align="center" width="1.5cm" border-bottom="1">
									<img src="/simwisuda/inc/page.jpg" style="width: 55; height: 10" />
									'.$halaman.' 
								</td>
								<td width="45%"></td>
					...
					<td width="2mm"></td>				
					<td width="4mm" height="180mm">					
						<br>
						<img src="/simwisuda/inc/sidekanan.png" style="width: 20; height: 1030" >
					</td>
					</tr>
					<tr>
